...
class AttackRequest(BaseModel):
    attack_method: Literal["ica","overload","past_tense","rene_llm","art_prompt","deep_inception","gpt4_cipher","pair","rewrite","random_search","tap"] = Field(..., description="Attack method, must be one of the supported methods")
    ...
    attack_model: str = Field(..., description="Attack model name")
    target_model: str = Field(..., description="Target model name")
    attack_payload: Optional[str] = Field(None, description="Custom attack payload")
    max_turns: int = Field(10, ge=1, le=100, description="Max conversation turns (1-100)")
    temperature: float = Field(0.7, ge=0.0, le=2.0, description="Generation temperature, range [0.0, 2.0]")
    custom_params: Optional[Dict[str, Any]] = Field(None, description="Optional custom parameters")
