...
class DeepInceptionAttackerConfig(BaseAttackerConfig):
    ...
    :param scene: The scene type for the inception attack. Default is "science fiction".
    :type scene: str
    ...
    scene: str = field(default="science fiction")
...
class DeepInceptionAttacker(BaseAttacker):
    ...
    def attack(self, messages: List[Dict[str, str]], **kwargs):
        ...
        attack_prompt = self.rewrite_template.format(
            scene=self.scene,
        ...
        logger.debug(f"The output messages of deep inception attack is:{messages}")
        logger.info(f"Get final messages from deep inception attack!!")
