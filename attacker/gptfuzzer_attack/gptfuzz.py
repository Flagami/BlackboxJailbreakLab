...
from attacker.gptfuzzer_attack.fuzzer.selection import MCTSExploreSelectPolicy
from attacker.gptfuzzer_attack.fuzzer.mutator import (
...
from attacker.gptfuzzer_attack.fuzzer.core import GPTFuzzer
from attacker.gptfuzzer_attack.utils.predict import RoBERTaPredictor
from utils import setup_logger

logger = setup_logger(__name__)

# TODO:需要用到本地部署的ROBERT这类模型做预测模型，所以属于特殊类型的黑盒
@dataclass
class GPTFuzzAttackerConfig(BaseAttackerConfig):
    ...
    attacker_name: str = field(default="GPTFuzz")
    attack_type: any = field(default=None)
    attacker_llm_config: BaseLLMConfig = field(default_factory=SiliconFlowLLMConfig)
    attacker_llm_gen_config: LLMGenerateConfig = field(default=LLMGenerateConfig)
    ...
    initial_seed: str = field(default="attacker/data/GPTFuzzer.csv")
...
class GPTFuzzAttacker(BaseAttacker):
    ...
    def __init__(self, config: GPTFuzzAttackerConfig):
        ...
        self.attack_llm = ModelInterface().create_llm(config=config.attacker_llm_config)
